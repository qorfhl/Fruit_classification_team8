...
# Argparse
def get_args():
    parser = argparse.ArgumentParser(description="ResNet50 FruitCNN Training")
    ...
    parser.add_argument("--output_dir",  type=str,   default="./outputs/resnet50")
    parser.add_argument("--epochs",      type=int,   default=30)
    parser.add_argument("--batch_size",  type=int,   default=32)
    parser.add_argument("--lr",          type=float, default=1e-4)
    parser.add_argument("--save_model",  action="store_true")
    parser.add_argument("--model_path",  type=str,   default="best_model_resnet50.pth")
...
def save_prediction_samples(model, loader, class_names, device, output_dir, n_samples=10):
    ...
    for tag, samples in [("correct", correct_saved), ("wrong", wrong_saved)]:
        ...
        for ax, (img, label, pred) in zip(axes, samples):
            ...
            ax.axis("off")
        plt.suptitle(
            f"{'Correct' if tag == 'correct' else 'Wrong'} Predictions (ResNet50)",
            fontsize=10
        )
        plt.tight_layout()
...
def main():
    ...
    print(f"\nModel: ResNet50 (pretrained) | Parameters: {num_params:,}")

    run_id    = f"resnet50_adam_lr{args.lr}_bs{args.batch_size}"
    ...
    plot_confusion_matrix(
        ...
        title="ResNet50 (pretrained) - Test Confusion Matrix",
    ...
    with open(results_csv, "a", newline="") as f:
        ...
        if not file_exists:
            writer.writerow(["model", "optimizer", "lr", "batch_size", "epochs",
                              ...
                              "test_acc", "test_f1"])
        writer.writerow(["ResNet50_pretrained", "adam", args.lr, args.batch_size, args.epochs,
