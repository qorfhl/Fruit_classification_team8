...
# Argparse
def get_args():
    parser = argparse.ArgumentParser(description="FruitCNN Baseline Training")
    ...
    parser.add_argument("--model_path",  type=str,   default="best_model_baseline.pth")
...
def save_prediction_samples(model, loader, class_names, device, output_dir, n_samples=10):
    ...
    for tag, samples in [("correct", correct_saved), ("wrong", wrong_saved)]:
        ...
        for ax, (img, label, pred) in zip(axes, samples):
            ...
            ax.axis("off")
        plt.suptitle(
            f"{'Correct' if tag == 'correct' else 'Wrong'} Predictions (Baseline CNN)",
            fontsize=10
        )
        plt.tight_layout()
...
def main():
    ...
    print(f"\nModel: Baseline FruitCNN | Parameters: {num_params:,}")

    run_id    = f"baseline_adam_lr{args.lr}_bs{args.batch_size}"
    ...
    plot_confusion_matrix(
        ...
        title="Baseline FruitCNN - Test Confusion Matrix",
    ...
    with open(results_csv, "a", newline="") as f:
        ...
        if not file_exists:
            writer.writerow(["model", "optimizer", "lr", "batch_size", "epochs",
                              ...
                              "test_acc", "test_f1"])
        writer.writerow(["BaselineCNN", "adam", args.lr, args.batch_size, args.epochs,
